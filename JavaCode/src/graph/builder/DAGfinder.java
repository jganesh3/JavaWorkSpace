...
public class DAGfinder {
	...
	private void DFS_VISIT(Graph g, Vertex u) {
		...
		// Add the vertex in the onStack 
		// onStack will maintains  all the current vertices present on the stack
		onStack.add(u);
		...
		while(iterator.hasNext()){
			...
			if(hasCycle()) return;
				...
				// If the vertex is already on Stack , this means this vertex is already visited and we found a back edge. 
				v.setParent(u);
				//Simply add this vertex on stack;
				//Here we are using actual Stack to trace the Vertices which are forming cycle.
				stack=new Stack<>();
				Vertex x=v;
				while(x!=u){
